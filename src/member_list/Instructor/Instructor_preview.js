...
const Instructor_list = () => {
  ...
  const [members, setMembers] = useState([]); // 강사 리스트
  ...
  const fetchMembers = async () => {
    if (loading || !hasMore) return;  // 로딩중이거나 데이터가 더이상 없으면 실행x

    setLoading(true);

    try {
      const response = await fetch(`https://localhost:443/instructor?page=${currPage}&pageSize=10`, {
        method: 'POST',
      });

      if (response.ok) {
        const data = await response.json();
        setMembers((prevMembers) => [...prevMembers, ...data.content]);
...
return (
  <div className={styles.main}>
    ...
    <table className={styles.tableBody}>
      <tbody>
        {members.map((member, index) => (
          <tr key={member.instructorId} className={styles.tr} ref={index === members.length - 1 ? lastElementRef : null}>
    ...
    {!hasMore && <p style={{ textAlign: 'center' }}>더 이상 데이터가 없습니다.</p>}
  </div>
);
};
